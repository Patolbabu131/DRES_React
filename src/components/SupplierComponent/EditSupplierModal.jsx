...
// Reuse validation rules from AddSupplierModal
...
const EditSupplierModal = ({ isOpen, onClose, onSuccess, supplier }) => {
  ...
  // Pre-populate form when supplier data is available
  useEffect(() => {
    if (supplier && isOpen) {
      setFormData({
        id: supplier.id || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Assuming your API expects supplier id along with form data
      const response = await ApiClient.updateSupplier(supplier.id, formData);
      if (onSuccess) onSuccess(response.data.data);
      onClose();
    } catch (error) {
      const serverMessage = error.response?.data?.message;
      const clientMessage = serverMessage || error.message || 'Failed to update supplier';
  ...
  // Base classes for inputs with responsive Tailwind classes
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-xl sm:text-2xl font-semibold mb-6">Edit Supplier</h2>
